  ...
  <nav class="z-50 sticky top-0 flex justify-between items-center p-4 backdrop-blur-md bg-black/25 text-white">
    ...
    <div class="flex items-right cart-container">
      <div class="relative">
        ...
        <div v-if="isCartOpen" ref="cartDropdown" class="z-50 absolute right-[-45%] top-14 w-[400px] text-white backdrop-blur-lg bg-black/30">
          <div class="max-h-[300px] overflow-y-auto">
            <ul v-for="(cartProduct, index) in cartStore.products" :key="index">
              <li>
                <div class="flex items-center">
                  <img class="p-2 max-h-16" :src="cartProduct.image" :alt="cartProduct.title">
                  <div class="p-2 grow">
                    <p class="text-lg">{{ cartProduct.title }}</p>
                    <p class="text-md bold">${{ cartProduct.price }}</p>
